namespace LibCPK
{
    internal class CriCompression
    {
        public unsafe static int CRIcompress(byte* dest, int* destLen, byte* src, int srcLen)
        {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (; ; )
                        {
                            for (; T >= 8;)
                            {
                                *(dest + m--) = (byte)((d >> (T - 8)) & 0xff); T -= 8; d = d & ((1 << T) - 1);
                            }
                            if (p < 255) break;
            ...
            *destLen = *destLen - m; dest += m;
            ...
            for (j = 0; j < 4; j++)
            {
                for (i = 0; i < 4; i++)
                {
                    *(odest + i + j * 4) = (byte)(l[j] & 0xff); l[j] >>= 8;
                }
            }
